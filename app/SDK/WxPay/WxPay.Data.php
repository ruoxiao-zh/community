...
class WxPayDataBase
{
	...
	* @param string $value 
	**/
	public function SetSign($pay_config)
	{
		$sign = $this->MakeSign($pay_config);
	...
	**/
	 ...
	 */
	public function MakeSign($pay_config)
	{
		//签名步骤一：按字典序排序参数
		ksort($this->values);
...
class WxPayResults extends WxPayDataBase
{
	 ...
	 * 检测签名
	 */
	public function CheckSign($pay_config)
	{
		//fix异常
		if(!$this->IsSignSet()){
	 ...
	 */
	public static function InitFromArray($pay_config, $array, $noCheckSign = false)
	{
		$obj = new self();
		$obj->FromArray($array);
		if($noCheckSign == false){
			$obj->CheckSign($pay_config);
		}
        return $obj;
	}
     ...
     * @throws WxPayException
     */
	public static function Init($pay_config, $xml)
	{	
		$obj = new self();
		$obj->FromXml($xml);
		//fix bug 2015-06-29
